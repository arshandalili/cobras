...
def load_tqa_nat_unsteered(
    model_name: str,
    layer_idx: int,
    split_idx: int,
) -> Tensor:
    """Load merged (all seeds) unsteered LLM completion activations for split_idx.

    Generated by scripts/extract_nat_unsteered_activations.py.
    """
    data_dir = get_project_dir() / 'data' / 'truthfulqa' / 'activations' / model_name
