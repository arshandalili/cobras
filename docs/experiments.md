...
| **Gate sweep** | Which bridge quantity should gate: the time marginal, the extended-potential density, or the drift norm | `scripts/analysis/ood_gate/sb_marginal_gate.py` | `confs/steer/GateSweep-*` | `experiments/gate_sweep.sh` |
...
The gate reads the bridge's own **time marginal** and nothing else.

The probability-flow drift of Eq. (13) is the *ratio* of the two Schrödinger potentials, in
which their common kernel decay in the distance to the data cancels exactly. The decay survives
only in the *product*, `p_0 = ψ̂ · φ̂` (Sec. 4.2) — the one factor of the SB solution the drift
never reads, and therefore the only one that can say whether the bridge assigned a query any
transport at all. The score is `-log p_0` at bandwidth `b = abstain_bandwidth_scale · σ²`.

That bandwidth picks a point in a one-parameter family the bridge already defines:
`-2b·log φ̂_b` is the *mean* squared geodesic distance to the negatives at `b = σ²`, and
converges to the *minimum* of that distance as `b → 0`. The shipped default, `σ²/256`, sits
near the small-bandwidth end; `scripts/analysis/ood_gate/sb_marginal_gate.py` is what fixes it.

**One knob.** The score is mapped through its own empirical CDF under the reference sample, so
`abstain_percentile` is a nominal in-distribution coverage, not a threshold on a raw quantity.
The gate is fully open for the most in-distribution fraction `p` and ramps linearly to zero
over the rest, which makes the mean in-distribution gate exactly `(1 + p) / 2`. The shipped
`p = 0.534` therefore leaves 0.767 of the in-distribution steering in place. There is no
sharpness, no neighbourhood size, and no threshold to tune.

**Calibrate on queries.** The contrastive pairs are *answer* activations; the gate is applied to
*prompt* activations, which sit at a different position and so at a systematically different
radius. `abstain_on_queries: true` calibrates on real query activations instead, which
`scripts/prepare/extract_query_activations.py` writes to `data/query_activations/`.

Measured on Llama3.1-8B-Base layer 13 at the shipped setting:

| query set | mean gate | AUROC vs in-distribution |
|---|---|---|
| TruthfulQA (in-distribution) | 0.768 | — |
| NQ | 0.340 | 0.838 |
| TriviaQA | 0.208 | 0.899 |
| GSM8K | 0.047 | 0.978 |
| MMLU | 0.000 | 1.000 |

The in-distribution column reproduces the closed form to three decimals.
`scripts/analysis/ood_gate/gate_calibration.py` regenerates this table.

The retired k-NN gate — a geodesic radius to the `abstain_k`-th nearest negative, thresholded
by `1/(1 + (ρ/ρ_ref)^sharpness)` — is gone entirely, along with `GatedSteer` and the
`*-Gate` baseline configs that existed to attach it to methods with no bridge of their own.
...
strings (`q1-cobras-nogate-...`) are the keys the cached files in `results/` are stored
...
## Results produced before the gate change

Replacing the gate changes every number that ran with abstention on. Cached results under the
old run names (`q1-cobras-knn-ratio-*`, `COBRAS-gate0.98-k32-sh200*`) are from the retired k-NN
gate and are not comparable with anything produced now; the configs that wrote them no longer
exist. Ungated rows (`abstain_percentile: 1.0`) are unaffected.
