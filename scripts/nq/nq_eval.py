...
nq_df_cols = ["Model", "Steering Method", "Accuracy", "N"]
...
def main():
    args = parse_args()
    output_dir = get_project_dir() / "results" / "nq"
    raw_dir = output_dir / "raw_outputs" / args.model
    eval_path = (
        output_dir / "eval_results"
        / f"{args.model}-l{args.layer_idx}-NQ-seed{args.seed}.csv"
    )
    eval_path.parent.mkdir(parents=True, exist_ok=True)

    eval_df = pd.read_csv(eval_path) if eval_path.exists() else pd.DataFrame(columns=nq_df_cols)

    prefix = f"{args.model}-l{args.layer_idx}-"
    suffix = f"-NQ-seed{args.seed}"
