...
def main(cfg: DictConfig):
    ...
    output_dir: Path = get_project_dir() / "results" / "triviaqa" / "raw_outputs" / cfg.model
    output_dir.mkdir(parents=True, exist_ok=True)
    filename = f"{cfg.model}-l{cfg.layer_idx}-{cfg.steer.name}-TriviaQA-seed{cfg.seed}.jsonl"
    if (output_dir / filename).exists():
        print(f"✓ Output file {filename} already exists. Skipping.")
        exit()

    try:
        print(f"→ Running TriviaQA OOD eval for {cfg.model}-{cfg.steer.name} on layer {cfg.layer_idx}")
        ...
        print("→ Loading TriviaQA data ...")
        questions, correct_answers, false_answers = load_triviaqa_data()
        ...
        print(f"→ Scoring {len(prompts)} TriviaQA questions with T={cfg.steer.T} ...")
        ...
        print(f"→ TriviaQA Accuracy: {accuracy:.4f}")
        ...
        print(f"✓ Completed {cfg.model}-{cfg.steer.name} on TriviaQA")
        print("-" * 120)
