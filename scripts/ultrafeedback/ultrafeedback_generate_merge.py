...
def main(cfg: DictConfig):
    ...
    input_dir: Path = get_project_dir() / "results" / "ultrafeedback" / "raw_rank_outputs" / cfg.model
    ...
    pattern = (
        ...
        f"-UltrafeedbackBinarized-seed{cfg.seed}-rank*-of-*.jsonl"
    )
    ...
    merged_path = output_dir / (
        f"{cfg.model}-l{cfg.layer_idx}-{cfg.steer.name}-UltrafeedbackBinarized-seed{cfg.seed}.jsonl"
    )
